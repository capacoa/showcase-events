...
const CELLS = {
  ...
  // The script scans downward until it finds the sentinel label in column A.
  artistFirstRow:  19,
  artistNameCol:   2,    // column B
  artistSameAsCol: 3,    // column C
  artistStopLabel: "Add lines above this",  // sentinel in column A
};
...
function generateJsonLD() {
  ...
  // Build venue sameAs: if the user entered a full URL use it directly,
  ...
  // ── Read performers ────────────────────────────────────────────────────────
  // Scans downward from artistFirstRow until the sentinel label is found in
  // column A, so any number of inserted rows will be picked up automatically.
  const performers = [];
  ...
  while (true) {
    ...
    if (label === CELLS.artistStopLabel) break;  // hit the sentinel row — stop
    if (row > 500) break;                        // safety cap, just in case
